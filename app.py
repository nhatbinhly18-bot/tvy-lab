...
import streamlit.components.v1 as components # 新增组件库引用
...
<style>
    ...
    /* 顶部标题区 - 恢复为您喜欢的原版 CSS 样式 */
    .saas-header {
        background: linear-gradient(135deg, #eff6ff 0%, #ffffff 100%);
        padding: 2rem;
        border-left: 6px solid #2563eb; /* 您的最爱：左侧品牌与左边框 */
        border-radius: 8px;
        margin-bottom: 2rem;
...
with st.sidebar:
    ...
    # 修改：增加了简报生成选项
    mode = st.radio("功能切换：", ["📝 领导公务单自动生成器", "✨ 智能简报生成", "🔍 龙华学校查号台"])
...
# ----------------- 模块二：智能简报生成 (新) -----------------
    ...
    st.caption("↖️ **导航：** 点击左上角 **>** 可切换功能")
    st.title("✨ 体卫艺 AI 简报创作中心")
    
    st.info("💡 正在连接 DeepSeek V3 智慧大脑... 审核通过后，此处将直接显示对话窗口。")
    
    # 嵌入 iframe
    # 这里的 URL 暂时用 baidu 占位，您可以随时替换为 Coze 的分享链接
